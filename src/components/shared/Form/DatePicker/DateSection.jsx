...
import { DateSection, YearSelect, SelectedMonthYear, Grid, PrevNextIcons, DayName, Day } from './styles'
    ...
    <DateSection>
      ...
      <Grid>
        ...
        {generateMonthDays(selectedMonth).map(date => (
          <Day key={date} isToday={moment().isSame(date, 'day')} isSelected={moment(value).isSame(date, 'day')} onClick={() => handleDayChange(date)}>
            {formatDate(date, 'D')}
          </Day>
        ))}
...
const generateYearOptions = () => [{ label: 'Year', value: '' }, ...times(51, i => ({ label: `${i + currentYear - 50}`, value: `${i + currentYear - 50}` }))]
...
const generateMonthDays = selectedMonth => times(selectedMonth.daysInMonth()).map(i => moment(selectedMonth).add(i, 'days'))
